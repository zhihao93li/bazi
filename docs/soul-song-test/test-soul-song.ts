...
1. **【情感暗区：被折叠的真相】**
   - 拒绝性格描述，直接白描其内心的“牺牲”。
   - 示例：不要说“你很坚强”，要说“你必须表现得像棵树，因为你身后空无一人，你连倒下的权利都没有”。
2. **【灵魂回响：那首懂你的歌】**
   - 歌曲：[歌名] - [歌手]
   - 核心台词：[选取一句极具宿命感的歌词]
3. **【被击中的瞬间：为什么是这一首？】**
   - 揭示歌曲旋律与命盘“情感压强”的重合。
   - 揭示歌词如何精准撞击了那个“不敢示人的念头”。
4. **【她赋 · 独白】**
   - 一句冷彻心扉却又极具尊严的寄语。
...
# Analysis Task
  ...
  - **灵魂匹配**：匹配一首能承接这种“巨大压强”的歌曲。这首歌不需要安慰他，只需要“确认为他”。
...
async function main() {
  ...
  const resultFile = path.join(resultsDir, `${timestamp}_results.md`);

  // 写入文件头
  const header = `# Soul Song 批量测试结果

**测试时间**: ${new Date().toLocaleString('zh-CN')}
**模型**: ${CONFIG.model}
**并发数**: ${CONFIG.concurrency}
**测试数量**: ${TEST_CASES.length}
...
`;
      ...
      if (error instanceof Error) {
        ...
        errorMsg = 'Unknown error (undefined/null)';
      } else {
